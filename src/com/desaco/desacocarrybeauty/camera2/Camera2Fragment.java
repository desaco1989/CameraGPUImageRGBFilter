...
public class Camera2Fragment extends Fragment
	 ...
	 * smallest one that is at least as large as the respective texture view
	 * size, and that is at most as large as the respective max size, and whose
	 * aspect ratio matches with the specified value. If such size doesn't
	 * exist, choose the largest one that is at most as large as the respective
	 * max size, and whose aspect ratio matches with the specified value.
	 *
	 * @param choices
	 *            The list of sizes that the camera supports for the intended
	 *            output class
	 * @param textureViewWidth
	 *            The width of the texture view relative to sensor coordinate
	 * @param textureViewHeight
	 *            The height of the texture view relative to sensor coordinate
	 * @param maxWidth
	 *            The maximum width that can be chosen
	 * @param maxHeight
	 *            The maximum height that can be chosen
	 * @param aspectRatio
	 *            The aspect ratio
	...
	private File getOutputMediaFile() {
		...
		return new File(picDir.getPath() + File.separator + "desaco_camera2_" + timeStamp + ".jpg");
	}
	...
	public void onRequestPermissionsResult(int requestCode, @NonNull String[] permissions,
		...
		if (requestCode == REQUEST_CAMERA_PERMISSION) {
			if (grantResults.length != 1 || grantResults[0] != PackageManager.PERMISSION_GRANTED) {
				ErrorDialog.newInstance("需要申请摄像头权限").show(getChildFragmentManager(), FRAGMENT_DIALOG);
			}
		}
		// TODO
		// else {
	...
	private void setUpCameraOutputs(int width, int height) {
		...
		try {
			...
			ErrorDialog.newInstance("该设备不支持Camera2API").show(getChildFragmentManager(), FRAGMENT_DIALOG);
		}
	}
	 ...
	 * in setUpCameraOutputs and also the size of `mTextureView` is fixed.
	 *
	 * @param viewWidth
	...
	/**
	 * Run the precapture sequence for capturing a still image. This method
	 * should be called when we get a response in {@link #mCaptureCallback} from
	 * {@link #lockFocus()}.
	 */
	...
	/**
	 * Capture a still picture. This method should be called when we get a
	 * response in {@link #mCaptureCallback} from both {@link #lockFocus()}.
	 */
	...
	public static class ConfirmationDialog extends DialogFragment {
		...
		public Dialog onCreateDialog(Bundle savedInstanceState) {
			...
			return new AlertDialog.Builder(getActivity()).setMessage("需要申请摄像头权限")
					.setPositiveButton(android.R.string.ok, new DialogInterface.OnClickListener() {
						@Override
