...
package com.desaco.desacocarrybeauty.gpu_image.gpu_sample;
...
import com.desaco.desacocarrybeauty.gpu_image.*;
...
public class GPUImageFilterTools {
    public static void showDialog(final Context context,
        ...
        filters.addFilter("Contrast 对比", FilterType.CONTRAST);
        filters.addFilter("Invert 倒置", FilterType.INVERT);
        filters.addFilter("Pixelation 像素化", FilterType.PIXELATION);
        filters.addFilter("Hue 色调", FilterType.HUE);
        filters.addFilter("Gamma 伽玛", FilterType.GAMMA);
        filters.addFilter("Brightness 亮度", FilterType.BRIGHTNESS);
        filters.addFilter("Sepia 乌贼", FilterType.SEPIA);
        filters.addFilter("Grayscale 灰度", FilterType.GRAYSCALE);
        filters.addFilter("Sharpness 锐度", FilterType.SHARPEN);
        filters.addFilter("Sobel Edge Detection 索贝尔边缘检测", FilterType.SOBEL_EDGE_DETECTION);
        filters.addFilter("3x3 Convolution 卷积", FilterType.THREE_X_THREE_CONVOLUTION);
        filters.addFilter("Emboss 浮雕", FilterType.EMBOSS);
        filters.addFilter("Posterize 色调分离", FilterType.POSTERIZE);
        filters.addFilter("Grouped filters 分组过滤器", FilterType.FILTER_GROUP);
        filters.addFilter("Saturation 饱和", FilterType.SATURATION);
        filters.addFilter("Exposure 曝光", FilterType.EXPOSURE);
        filters.addFilter("Highlight Shadow 突出显示阴影", FilterType.HIGHLIGHT_SHADOW);
        filters.addFilter("Monochrome 单色", FilterType.MONOCHROME);
        filters.addFilter("Opacity 不透明度", FilterType.OPACITY);
        filters.addFilter("RGB", FilterType.RGB);
        filters.addFilter("White Balance 白平衡", FilterType.WHITE_BALANCE);
        filters.addFilter("Vignette 小插图", FilterType.VIGNETTE);
        filters.addFilter("ToneCurve 音调曲线", FilterType.TONE_CURVE);

        filters.addFilter("Blend (Difference) 混合差异", FilterType.BLEND_DIFFERENCE);
        filters.addFilter("Blend (Source Over) 来源", FilterType.BLEND_SOURCE_OVER);
        filters.addFilter("Blend (Color Burn) 颜色烧伤", FilterType.BLEND_COLOR_BURN);
        filters.addFilter("Blend (Color Dodge) 颜色道奇", FilterType.BLEND_COLOR_DODGE);
        filters.addFilter("Blend (Darken) 变暗", FilterType.BLEND_DARKEN);
        ...
        filters.addFilter("Blend (Hard Light) 硬光", FilterType.BLEND_HARD_LIGHT);
        filters.addFilter("Blend (Lighten) 减轻", FilterType.BLEND_LIGHTEN);
        filters.addFilter("Blend (Add) 加", FilterType.BLEND_ADD);
        filters.addFilter("Blend (Divide)除", FilterType.BLEND_DIVIDE);
        ...
        filters.addFilter("Blend (Alpha)透明", FilterType.BLEND_ALPHA);
        filters.addFilter("Blend (Color)颜色", FilterType.BLEND_COLOR);
        filters.addFilter("Blend (Hue)色调", FilterType.BLEND_HUE);
        filters.addFilter("Blend (Saturation)饱和", FilterType.BLEND_SATURATION);
        filters.addFilter("Blend (Luminosity)亮度", FilterType.BLEND_LUMINOSITY);
        filters.addFilter("Blend (Linear Burn)线性烧伤", FilterType.BLEND_LINEAR_BURN);
        filters.addFilter("Blend (Soft Light)柔光", FilterType.BLEND_SOFT_LIGHT);
        ...
        filters.addFilter("Blend (Normal)正常", FilterType.BLEND_NORMAL);

        filters.addFilter("Lookup (Amatorka)阿马替迦", FilterType.LOOKUP_AMATORKA);
        filters.addFilter("Gaussian Blur 高斯模糊", FilterType.GAUSSIAN_BLUR);
        filters.addFilter("Crosshatch 十字架", FilterType.CROSSHATCH);

        filters.addFilter("Box Blur 盒模糊", FilterType.BOX_BLUR);
        filters.addFilter("CGA Color Space 颜色空余", FilterType.CGA_COLORSPACE);
        filters.addFilter("Dilation 扩张", FilterType.DILATION);
        filters.addFilter("Kuwahara 桑原", FilterType.KUWAHARA);
        filters.addFilter("RGB Dilation 扩张", FilterType.RGB_DILATION);
        filters.addFilter("Sketch 素描", FilterType.SKETCH);
        filters.addFilter("Toon 香椿", FilterType.TOON);
        filters.addFilter("Smooth Toon 光滑的香椿", FilterType.SMOOTH_TOON);
        filters.addFilter("Halftone 半色调", FilterType.HALFTONE);

        filters.addFilter("Bulge Distortion 膨胀变形", FilterType.BULGE_DISTORTION);
        filters.addFilter("Glass Sphere 玻璃球", FilterType.GLASS_SPHERE);
        filters.addFilter("Haze 阴霾", FilterType.HAZE);
        filters.addFilter("Laplacian 拉普拉斯", FilterType.LAPLACIAN);
        filters.addFilter("Non Maximum Suppression 非最大抑制", FilterType.NON_MAXIMUM_SUPPRESSION);
        filters.addFilter("Sphere Refraction 球体折射", FilterType.SPHERE_REFRACTION);
        filters.addFilter("Swirl 漩涡", FilterType.SWIRL);
        filters.addFilter("Weak Pixel Inclusion 弱像素包容", FilterType.WEAK_PIXEL_INCLUSION);
        filters.addFilter("False Color 假色", FilterType.FALSE_COLOR);

        filters.addFilter("Color Balance 色调平衡", FilterType.COLOR_BALANCE);
        filters.addFilter("Levels Min (Mid Adjust) 最小等级", FilterType.LEVELS_FILTER_MIN);
        filters. addFilter("Bilateral Blur 双边模糊", FilterType.BILATERAL_BLUR);
        filters.addFilter("Transform (2-D) 转变", FilterType.TRANSFORM2D);
